...
# OSD Status
OSD_STATUS=$(ceph -s | grep osd | awk '{print "UP:"$5 "    " "IN:"$7}')
purple "OSD状态"
green "$OSD_STATUS"

# OSD Usage
OSD_USE=$(ceph osd df tree | grep -v host | grep -v MIN | grep -v TOTAL |  ceph osd df tree | grep -v host | grep -v MIN | grep -v TOTAL | awk 'NR!=1&&NR!=2{print $10 "   " $7}')
purple "OSD空间使用率"
green "$OSD_USE"

# Cluster Space Utilization
CLUSTER_SPACE=$(ceph -s | grep used | awk '{print "Total:"$7$8 "   " "Used:"$1$2}')
purple "集群空间使用量"
green "$CLUSTER_SPACE"
